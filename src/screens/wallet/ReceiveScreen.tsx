    ...
    return (
        <SafeAreaView className="flex-1 bg-baseBg">
            <View className="flex-1 justify-center items-center">
                <View className='border-4 border-green-600 border-solid rounded-lg p-4 bg-white'>
                    <QRCode
                        backgroundColor={Colors.primary}
                        value={walletAddress ?? ""}
                        size={Dimensions.get('window').width * 0.5}
                        // logo={Images.EthToken}
                        logoSize={50}
                        logoBorderRadius={50}
                ...
                <CustomText className='text-primary font-PlusJakartaSans text-md mx-2 mt-4 text-center mb-10'>
                    This address can only be used to receive compatible tokens in the eclipse ecosystem.
                </CustomText>
            </View>
        </SafeAreaView>
    )

...
export function KeyboardDismissPressable(props: ViewProps) {
    return (
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()} accessible={false}>
            <SafeAreaView style={{ paddingBottom: getAPILevel() >= 34 ? 30 : 0 }} {...props} />
        </TouchableWithoutFeedback>
    )
}
...
export function CircularTimer(props: CircularTimerProps) {  
    const { countdown, duration = 60, color = "#4ade80", tintColor = Colors.primary } = props
    return (
        <View className="items-center justify-center">
            <AnimatedCircularProgress
                size={20}
                width={2}
                fill={((duration - countdown) / duration) * 100}
                tintColor={tintColor}
                backgroundColor={color}
                rotation={0}
                lineCap="round"
            >
                {/* {() => <CustomText className="text-primary text-xs">{countdown}s</CustomText>} */}
            </AnimatedCircularProgress>
        </View>
    );
};
